    ...
    SubShader
    {
        ...
        /**Pass
        {
            ...
            half4 frag(Varyings i): SV_Target
            {
                ...
                half horizonControlFactor = pow(1 - i.uv.y, _HorizonContract / min(abs(_TimeMapping * 10) + 0.5 - _ShiftHorizonColorRampBias, 1)) * saturate(i.uv.y) * _HorizonStrength;

                _SunsetHorizonColor += half4((_DayHorizonColor.rgb - _SunsetHorizonColor.rgb) * pow(_SunsetHorizonColorRamp, 5) * saturate(_TimeMapping), 0);
                ...
                half3 horizionColor =dayHorizonColor + nightHorizonColor;
                horizionColor *= min(pow(_TimeMapping * 20, 4), _ShiftHorizonIntensityRamp) + (1 - _ShiftHorizonIntensityRamp);
        ...
        Pass
		{
			...
			fixed4 frag(Varyings i): SV_Target
			{
                ...
                fixed horizonControlFactor = pow(1 - yFactor, _HorizonContract / min(abs(_TimeMapping * 10) + 0.5 - _ShiftHorizonColorRampBias, 1)) * saturate(yFactor) * _HorizonStrength;

                _SunsetHorizonColor += fixed4((_DayHorizonColor.rgb - _SunsetHorizonColor.rgb) * pow(_SunsetHorizonColorRamp, 5) * saturate(_TimeMapping), 0);
                ...
                fixed3 horizionColor =dayHorizonColor + nightHorizonColor;
                horizionColor *= min(pow(_TimeMapping * 20, 4), _ShiftHorizonIntensityRamp) + (1 - _ShiftHorizonIntensityRamp);
